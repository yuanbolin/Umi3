...
class UserEdit extends Component {
  ...
  getDetail = () => {
    get(`sys/user-employee/${this.props.match.params.id}`).then(res => {
      ...
      let initVal = { ...data, postInfoList: newpostInfoList, companyInfo: { id: data.companyInfo.id }, officeInfo: { id: data.officeInfo.id } }
      ...
      this.setState({ officePostInfoList: data.officePostInfoList, selectedRowKeys })
    })
  }
  ...
  addOneFushu = () => {
    ...
    this.setState(prevState => ({ officePostInfoList: prevState.officePostInfoList.concat(obj) }))
  }

  removeOneFushu = index => {
    this.setState(prevState => ({ officePostInfoList: prevState.officePostInfoList.filter(item => item.id !== index) }))
  }
                  ...
                  {officePostInfoList.map((item, i) => {
                    return (
                      <Row gutter={16} key={item.id}>
                        ...
                        <Col span={10}>
                          <Form.Item>
                            {getFieldDecorator(`fushujigou-${item.id}`, {
                              rules: [
                                {
                                  required: true,
                                  message: '必填!',
                        ...
                        <Col span={10}>
                          <Form.Item>
                            {getFieldDecorator(`fushugangwei-${item.id}`, { initialValue: item.post ? item.post.id : null })(
